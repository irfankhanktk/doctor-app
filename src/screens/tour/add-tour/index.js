...
import {setTourForEdit} from 'store/reducers/tour-reducer';
import Bold from 'typography/bold-text';
import Regular from 'typography/regular-text';
import {UTILS} from 'utils';
import {addTourValidation} from 'validations';
...
const AddTour = props => {
  ...
  const {values, errors, touched, setFieldValue, setFieldTouched, isValid} =
    useFormik({
      ...
      validationSchema: addTourValidation,
      onSubmit: () => {},
    });
  React.useEffect(() => {
    if (route?.params?.id) {
      dispatch(getTourForEdit(route?.params?.id, setLoading));
    } else {
      setLoading(false);
    }
  }, [route?.params?.id]);
  React.useEffect(() => {
    if (edit_tour && route?.params?.id) {
      ...
      setFieldValue('video', edit_tour?.row?.video);
      setFieldValue('banner_image_id', edit_tour?.row?.banner_image_id);
      setFieldValue('image_id', edit_tour?.row?.image_id);
      setFieldValue('gallery', edit_tour?.row?.gallery);
    }
  }, [edit_tour]);
  React.useEffect(() => {
    dispatch(getTourAttributes());
  }, []);
  React.useEffect(() => {
    dispatch(setTourForEdit(null));
  }, []);
  ...
  return (
    <View style={styles.container1}>
      <Header1x2x
        title={t(route?.params?.id ? 'edit_tour' : 'add_tour')}
        back={true}
      />
        ...
        <KeyboardAvoidScrollview
          ...
          {route?.params?.id ? (
            <PrimaryButton
              ...
              onPress={() =>
                navigate('EditTourAvailability', {tour_id: route?.params?.id})
              }
              title={t('edit_tour_availability')}
            />
          ) : (
            <></>
          )}
