...
import {
  onAddAvailability,
  onEditHospitalAvailbilityDetails,
  updateDoctorAvailabilityDaysTime,
...
const UpdateAvailability = props => {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        const res = await onEditHospitalAvailbilityDetails(
          hospital_id,
          userInfo?.id,
        );
  ...
  return (
    <View style={styles.container}>
      <Header1x2x
        title={`${hospitals?.find(h => h?.id === hospital_id)?.title}`}
      />
      {screenLoading ? (
        <Loader />
      ) : (
