...
import EventAvailabilityModal from 'components/molecules/event/modals/availability-event-modal';

const EditEventAvailability = props => {
  ...
  const getAvailabilty = async () => {
    try {
      ...
      if (events?.length) throw new Error('You have no event against event id');
      const res = await getEventAvailability(
        events?.data[0]?.id,
  ...
  const updateAvailabilty = async () => {
    try {
      ...
      const res = await updateEventAvailability(data);
  ...
  return (
    <View style={styles.container1}>
      <Header1x2x title={t('event_availability')} back={true} />
      <KeyboardAvoidScrollview
        contentContainerStyle={styles.contentContainerStyle}>
        <InputWithIcon
          value={events?.data?.find(x => x?.id == eventId)?.title}
...
export default EditEventAvailability;
