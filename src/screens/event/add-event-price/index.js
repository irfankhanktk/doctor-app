...
import {addPriceEventValidation} from 'validations';
import styles from './styles';

const AddEventPrice = props => {
  ...
  const initialValues = {
    ...
    default_state: `${edit_event?.row?.default_state || '0'}`,
    number: `${edit_event?.row?.number || ''}`,
    min_day_before_booking: `${edit_event?.row?.min_day_before_booking || ''}`,
    min_day_stays: `${edit_event?.row?.min_day_stays || ''}`,
    price: `${edit_event?.row?.price || ''}`,
    sale_price: `${edit_event?.row?.sale_price || ''}`,
    enable_extra_price: `${edit_event?.row?.enable_extra_price || '0'}`,
    extra_price: edit_event?.row?.extra_price || [
      {
        name: '',
        price: '',
        type: '',
    ...
    enable_service_fee: `${edit_event?.row?.enable_service_fee || '0'}`,
    service_fee: edit_event?.row?.service_fee || [
      {
        name: '',
        desc: '',
        price: '',
  ...
  const {values, errors, touched, setFieldValue, setFieldTouched, isValid} =
    useFormik({
      ...
      validationSchema: addPriceEventValidation,
      onSubmit: () => {},
    });
  const onSubmit = async () => {
    try {
      if (isValid && Object.keys(touched).length > 0) {
        try {
          navigation?.navigate('AddEventAttributes', {...values});
        } catch (error) {
          console.log(error);
        }
      } else {
        ...
        setFieldTouched('event_rating', true);
    ...
    <View style={styles.container}>
      ...
      <KeyboardAvoidScrollview
        ...
        <PrimaryInput
          ...
          label={t('sale_price')}
          placeholder={t('event_sale_price')}
