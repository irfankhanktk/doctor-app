...
import DoctorAvailability from 'components/molecules/doctor-availability-card';
import DoctorPersonalInfo from 'components/molecules/doctor-personal-info';
import DoctorQualification from 'components/molecules/doctor-qualification';
import { EmptyList } from 'components/molecules/empty-list';
import AppointmentLocationModal from 'components/molecules/modals/app-location-modal';
...
import { getAllDoctorsByCatId } from 'services/api/api-actions';
...
const SpecialistDetails = (props) => {
  ...
  return (
    <View style={styles.container}>
      <Header1x2x onChangeText={setSearchTerm} title={title} isSearch placeholder={t('search_here')} />
      {loading ? <Loader /> :
        <View style={{ flex: 1 }}>
          <Medium label={`Top ${doctors?.length} ${title}`} fontSize={mvs(18)} style={styles.heading} />
          <FlatList
            ...
            renderItem={({ item, index }) => (
              <View style={styles.card}>
                ...
                />
                <DescriptionCard description={item?.short_description || 'Description will be shown here'} />
                <ScrollView showsHorizontalScrollIndicator={false} horizontal>
                  {item?.locations?.map((ele, index) => <DoctorAvailability hospitals={ele?.hospitals} item={item} price={item?.price} key={index} />)}
                </ScrollView>
                <PrimaryButton
                  onPress={() => getLocations(item?.locations?.map(x => ({ ...x, price: item?.price })))}
                  containerStyle={{ marginTop: mvs(20) }}
                  title={t('book_appointment')} />
                {item?.is_featured && <View style={styles.heartContainer}>
                  <AntDesign name={'hearto'} color={colors.white} size={mvs(19)} />
                </View>}
              </View>
            )}
      ...
      <AppointmentLocationModal locationsLoading={locationsLoading} items={locations} visible={appointmentModal} onClose={(item) => {
        setAppointmentModal(false);
        console.log('item?', item);
        if (item)
          props?.navigation?.navigate('TimeSlot', item)
      }} />
    </View>
  );
};
