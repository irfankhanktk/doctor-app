...
import SlotDatePicker from 'components/molecules/date-slot-picker';
import DoctorPersonalInfo from 'components/molecules/doctor-personal-info';
import TimeSlotPicker from 'components/molecules/time-slot-picker';
...
import { getDoctorSlots, onDoctorBooking } from 'services/api/api-actions';
...
const TimeSlotScreen = (props) => {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const res = await getDoctorSlots(item?.doctor_id, item?.hospital_id, moment().add(selectedIndex, 'day').format('DD-MM-YYYY'));
  ...
  const onConfirmBookingPress = async () => {
    ...
    Alert.alert(
      'Confirm Booking',
      'Are you sure to confirm booking?',
      [
        {
          text: 'Cancel',
        ...
        {
          text: 'Confrim', onPress: () => dispatch(onDoctorBooking({
        ...
        <View style={styles.container}>
          ...
          <View style={styles.button}>
            <PrimaryButton
              ...
              onPress={() => userInfo?.id ? onConfirmBookingPress() : props?.navigation?.navigate('Login')}
