...
import DoctorAvailabilityCard from 'components/molecules/doctor-availability-card';
import DoctorHospitalCard from 'components/molecules/doctor-hospital-card';
import DoctorHospitalDetailsCard from 'components/molecules/doctor-hospital-details-card';
...
import { getDoctorAvailabilityDetails, onDeleteAvailbility } from 'services/api/api-actions';
...
const AvailabilityDetails = (props) => {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        const res = await getDoctorAvailabilityDetails(userInfo?.id, props?.route?.params?.hospital_id);
  ...
  return (
    <View style={styles.container}>
      <Header1x2x title={t("availability_details")} />
      {loading ?
        <Loader />
        : <>
