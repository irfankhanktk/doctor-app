...
import {addPriceTourValidation} from 'validations';
import styles from './styles';

const AddTourPrice = props => {
  ...
  const initialValues = {
    ...
    default_state: `${edit_tour?.row?.default_state || '0'}`,
    number: `${edit_tour?.row?.number || ''}`,
    min_day_before_booking: `${edit_tour?.row?.min_day_before_booking || ''}`,
    min_day_stays: `${edit_tour?.row?.min_day_stays || ''}`,
    price: `${edit_tour?.row?.price || ''}`,
    sale_price: `${edit_tour?.row?.sale_price || ''}`,
    enable_extra_price: `${edit_tour?.row?.enable_extra_price || '0'}`,
    extra_price: edit_tour?.row?.extra_price || [
      {
        name: '',
        price: '',
        type: '',
    ...
    enable_service_fee: `${edit_tour?.row?.enable_service_fee || '0'}`,
    service_fee: edit_tour?.row?.service_fee || [
      {
        name: '',
        desc: '',
        price: '',
  ...
  const {values, errors, touched, setFieldValue, setFieldTouched, isValid} =
    useFormik({
      ...
      validationSchema: addPriceTourValidation,
      onSubmit: () => {},
    });
  const onSubmit = async () => {
    try {
      if (isValid && Object.keys(touched).length > 0) {
        try {
          navigation?.navigate('AddTourAttributes', {...values});
        } catch (error) {
          console.log(error);
        }
      } else {
        ...
        setFieldTouched('tour_rating', true);
