  ...
  return (
    <TouchableOpacity
      ...
      <Row style={{ justifyContent: 'flex-start' }}>
        <Image source={item?.doctor?.banner_image_id ? { uri: item?.doctor?.banner_image_id } : login_bg} style={styles.img} />
        <View style={styles.leftContainer}>
          <Medium label={item?.patient?.name} style={styles.name} />
          <Regular label={`${item?.patient?.city || ''} ${item?.patient?.state || ''} ${item?.patient?.country || ''}`} style={styles.address} />
        </View>
      </Row>
      <Row style={{ ...styles.timeRow }}>
        ...
        <Row style={styles.alignItems}>
          ...
          <Medium label={moment(item?.date).format('ll')} style={styles.slotTime} />
        </Row>
      </Row>
      <Row style={{ justifyContent: 'flex-start', marginTop: mvs(10) }}>
        <Image source={{ uri: item?.hospital?.banner_image_id }} style={styles.img} />
        <View style={styles.leftContainer}>
      ...
      <Row style={{ backgroundColor: colors.lightGray, paddingHorizontal: mvs(5), borderRadius: mvs(5) }}>
        <Medium label={'Appointment status:'} />
        <Regular label={item?.status} />
      </Row>
      <Row style={{
        marginTop: mvs(15), alignItems: 'center',
      }}>
