...
import {setEventForEdit} from 'store/reducers/event-reducer';
...
const AddEvent = props => {
  ...
  React.useEffect(() => {
    if (route?.params?.id) {
      dispatch(getEventForEdit(route?.params?.id, setLoading));
    } else {
      setLoading(false);
    }
  }, [route?.params?.id]);
  React.useEffect(() => {
    if (edit_event && route?.params?.id) {
      // console.log('edit_event?.row?.popp->>', edit_event?.row?.faqs);
      setFieldValue('title', edit_event?.row?.title);
      setFieldValue('content', edit_event?.row?.content);
      setFieldValue('faqs', [...edit_event?.row?.faqs]);
      setFieldValue('video', edit_event?.row?.video);
      setFieldValue('banner_image_id', edit_event?.row?.banner_image_id);
      setFieldValue('image_id', edit_event?.row?.image_id);
      setFieldValue('gallery', edit_event?.row?.gallery);
    }
  }, [edit_event]);
  React.useEffect(() => {
    dispatch(getEventAttributes());
  }, []);
  React.useEffect(() => {
    dispatch(setEventForEdit(null));
  }, []);
  ...
  return (
    <View style={styles.container1}>
      <Header1x2x
        title={t(route?.params?.id ? 'edit_event' : 'add_event')}
        back={true}
      />
        ...
        <KeyboardAvoidScrollview
          ...
          {route?.params?.id ? (
            <PrimaryButton
              ...
              onPress={() =>
                navigate('EditEventAvailability', {event_id: route?.params?.id})
              }
              title={t('edit_event_availability')}
            />
          ) : (
            <></>
          )}
